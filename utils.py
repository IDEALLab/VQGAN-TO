...
# Assumes logits are input (NOT probabilities)
...
# Assumes probabilities are input and if not, clamps them to [0,1]
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        # Clamp inputs with an epsilon to avoid log(0)
        eps = 1e-6
...
class FocalActivationWrapper(torch.nn.Module):
    def __init__(self, use_focal_loss):
        super().__init__()
